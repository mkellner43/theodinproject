...
describe BinaryGame do
  ...
  describe '#player_input' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        incorrect_value = 's'
        incorrect_value_two = 't'
        valid_input = '3'
        allow(game_input).to receive(:gets).and_return(incorrect_value, incorrect_value_two, valid_input)
      end
  ...
  describe '#verify_input' do
    ...
    subject(:user_input) { described_class.new(1, 9) }
    # Note: #verify_input will only return a number if it is between?(min, max)

    context 'when given a valid input as argument' do
      it 'returns valid input' do
        min = user_input.instance_variable_get(:@minimum)
        max = user_input.instance_variable_get(:@maximum)
        user_input_value = 5
        expect(user_input.verify_input(min, max, user_input_value)).to be 5
      end
    end
    ...
    context 'when given invalid input as argument' do
      it 'returns nil' do
        ...
        max = user_input.instance_variable_get(:@maximum)
        user_input_value = 12
        expect(user_input.verify_input(min, max, user_input_value)).to be nil
      end
    end
  end
