...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:white_claw) { described_class.new('alcohol', 16)}

      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(white_claw.type).to eq('alcohol')
      end

      # remove the 'x' before running this test
      it 'has 16 ounces' do
        expect(white_claw).to be_full
      end
    end
  end
  ...
  describe '#full?' do
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:white_claw) { described_class.new('alcohol', 10) }
      # remove the 'x' before running this test
      it 'is not full' do
        expect(white_claw).not_to be_full
      end
    end
  end
end
